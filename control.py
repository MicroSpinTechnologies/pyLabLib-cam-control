...
class StandaloneFrame(container.QWidgetContainer):
    ...
    def setup(self, settings, cam_name, cam_desc):
        ...
        with self.using_new_sublayout("plots", "vbox"):
            ...
            self.image_proc_indicator.setup([("binning", ProcessingIndicator_ctl.binning_item(preprocess_thread)), ("background", ProcessingIndicator_ctl.background_item(process_thread))])
        ...
        self.add_property_element("defaults/window/size", lambda: (self.size().width(), self.size().height()), lambda v: self.resize(*v), add_indicator=False)
        self.add_property_element("defaults/window/position", lambda: (self.geometry().x(), self.geometry().y()), lambda v: self.setGeometry(v[0], v[1], self.size().width(), self.size().height()), add_indicator=False)
        self.add_property_element("defaults/window/maximized", lambda: self.windowState() == QtCore.Qt.WindowMaximized, lambda v: self.showMaximized() if v else None, add_indicator=False)
    ...
    def _add_param_loading(self, parent):
        ...
        with self.params_loading_settings.using_new_sublayout("scope", "hbox"):
            ...
            self.params_loading_settings.add_dropdown_button(
                "extras", "Extra...", options=["Tutorial", "Create camera shortcut", "Preferences", "About"], index_values=["tutorial", "cam_shortcut", "settings_editor", "about"]
            )
    ...
    def on_load_settings_button(self):
        path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, "Load GUI settings...", filter="Config Files (*.cfg);;Data Settings Files (*.dat);;All Files (*)", **{qtkwargs.file_dialog_dir: self.v["defaults/settings_folder"]}
        )
        if path:
    ...
    def on_save_settings_button(self):
        path, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "Save GUI settings...", filter="Config Files (*.cfg);;Data Settings Files (*.dat);;All Files (*)", **{qtkwargs.file_dialog_dir: self.v["defaults/settings_folder"]}
        )
        if path:
            self.v["defaults/settings_folder"] = os.path.split(path)[0]
    ...
    def create_camera_shortcut(self):
        if not win32com_present:
            ...
            return
        path, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Create camera shortcut...", filter="Shortcuts (*.lnk);;All Files (*)", **{qtkwargs.file_dialog_dir: os.path.expanduser("~\\Desktop")})
        if path:
    ...
    def load_plugin(self, plugin_class, name="__default__", parameters=None, start_order=0):
        ...
        plugin_ctl = plugins.PluginThreadController(
            "plugin.{}.{}".format(*full_name), kwargs={"name": name, "main_frame": self, "notifier": notifier, "plugin_cls": plugin_class, "ext_controller_names": self._ext_controller_names, "parameters": parameters}
        )
...
class MissingSettingsFrame(param_table.ParamTable):
    def setup(self, path):
        ...
        self.selected = False  # prevents double-call on multiple clicks
        w = self.add_decoration_label("Would you like to import config from a previous version, or to continue with camera detection?", location=(0, 0, 1, "end"))
        w.setWordWrap(True)
...
def start_threads(settings, cam_desc):
    ...
    services.FrameSaveThread(
        save_thread, kwargs={"src": preprocess_thread, "tag": "frames/new", "settings_mgr": settings_manager_thread, "frame_processor": process_thread, "garbage_collector": garbage_collector_thread}
    ).start()
...
def start_app(ask_on_no_cam=True):
    ...
    if len(cams) == 0:  # Show a message and exit
        ...
        QtWidgets.QMessageBox.warning(splash_screen, "No cameras", "No cameras detected. Make sure that the cameras are connected, turned on, and not used by other software.", QtWidgets.QMessageBox.Ok)
